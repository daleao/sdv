...
internal sealed class EventManager
{
    ...
    /// <summary>Implicitly manages all <see cref="IManagedEvent"/> types in the assembly.</summary>
        ...
        where TAttribute : Attribute
    {
        ...
        this.ManageImplicitly(t => t.HasAttribute<TAttribute>());
    }
    ...
    /// <param name="eventType">A <see cref="IManagedEvent"/> type to enable.</param>
    /// <returns><see langword="true"/> if the event's enabled status was changed, otherwise <see langword="false"/>.</returns>
    internal bool EnableForScreen(int screenId, Type eventType)
    {
    ...
    internal void EnableForScreen(int screenId, params Type[] eventTypes)
    {
        for (var i = 0; i < eventTypes.Length; i++)
        {
            this.EnableForScreen(screenId, eventTypes[i]);
        }
    }
        ...
        var eventTypes = AccessTools
            ...
            .Where(t => t.IsAssignableTo(typeof(IManagedEvent)) && !t.IsAbstract && predicate(t) &&
                        ...
                        (t.IsAssignableToAnyOf(typeof(GameLaunchedEvent), typeof(FirstSecondUpdateTickedEvent)) ||
                         t.HasAttribute<AlwaysEnabledEventAttribute>() || t.GetProperty(nameof(IManagedEvent.IsEnabled))?.DeclaringType == t))
            .ToArray();

        Log.T($"[EventManager]: Found {eventTypes.Length} event classes that should be enabled.");
        if (eventTypes.Length == 0)
        {
            return;
        }

        Log.T("[EventManager]: Instantiating events....");
        for (var i = 0; i < eventTypes.Length; i++)
        {
    ...
    private IManagedEvent? GetOrCreate(Type eventType)
    {
        ...
        if (instance is null)
        {
            Log.W($"[EventManager]: Failed to create {eventType.Name}.");
            return null;
        }

        this._eventCache.Add(eventType, instance);
        Log.T($"[EventManager]: Now managing {eventType.Name}.");
    ...
    private IManagedEvent? Create(Type eventType)
    {
        ...
        {
            return (IManagedEvent)eventType.RequireConstructor(this.GetType()).Invoke(new object?[] { this });
        }
