...
internal static class MonsterExtensions
{
    ...
    internal static bool IsGlider(this Monster monster)
    {
    ...
    internal static bool CanBeDamaged(this Monster monster)
    {
        return !monster.IsInvisible && !monster.isInvincible()
                && (monster is not Bug bug || !bug.isArmoredBug.Value)
                && (monster is not LavaCrab || monster.Sprite.currentFrame % 4 != 0)
    ...
    internal static bool IsWithinCharacterThreshold(this Monster monster, Character? other = null)
    {
        other ??= Game1.player;
