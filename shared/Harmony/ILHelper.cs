...
public sealed class ILHelper
{
    ...
    /// <param name="nth">Match the nth occurrence of this <paramref name="pattern"/>.</param>
    /// <returns>The <see cref="ILHelper"/> instance.</returns>
    public ILHelper CountUntil(CodeInstruction[] pattern, out int count, SearchOption search = SearchOption.Next, int nth = 1)
    {
        count = 0;
        this.Match(pattern, search, nth);
        for (var i = 0; i < nth; i++)
        {
    ...
    /// <param name="index">The <see cref="int"/> index of the stack pointer's position.</param>
    /// <param name="pattern">A pattern of <see cref="CodeInstruction"/>s to match.</param>
    /// <returns>
