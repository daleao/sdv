...
{
    /// <summary>Gets the unique ID of the initialized mod.</summary>
    internal static string? ModId { get; private set; }

    /// <summary>Initializes the <see cref="ModDataIO"/> using the namespace of the caller.</summary>
    /// <remarks>Should be called in <see cref="Mod.Entry"/>.</remarks>
    public static void Init()
    {
        ModId = new StackTrace().GetFrame(1)?.GetMethod()?.DeclaringType?.Namespace ??
                ThrowHelper.ThrowMissingMemberException<string>("Failed to initialize ModDataIO.");
    }
    ...
    public static void Write(Farmer farmer, string field, string? value)
    {
        ...
        Log.D("[ModData]: New data state:" +
              Game1.player.modData.ToDebugString(key => ModId is null || key.Contains(ModId)));
    }
    ...
    public static void Append(Farmer farmer, string field, string value, string separator = ",")
    {
        ...
        {
            Log.V($"[ModData]: {farmer.Name}'s {field} already contained {value}.");
        }
        else
        {
            Write(farmer, field, string.IsNullOrEmpty(current) ? value : current + separator + value);
            Log.V($"[ModData]: Appended {farmer.Name}'s {field} with {value}");
        }
    }
        ...
        where T : struct
    {
        ...
        Log.V($"[ModData]: Incremented {farmer.Name}'s {field} by {amount}.");
    }
        ...
        where T : struct
    {
        ...
        Log.V($"[ModData]: Incremented {farmer.Name}'s {field} by 1.");
    }
