...
internal sealed class FarmerTakeDamagePatcher : HarmonyPatcher
{
    ...
    {
        var weaponResilience = 0f;
        if (farmer.CurrentTool is { } tool and (MeleeWeapon or Slingshot))
        {
            weaponResilience += tool.Read<float>(DataFields.ResonantDefense);
            if (tool is MeleeWeapon weapon)
            {
                weaponResilience += weapon.addedDefense.Value;
            }
        }

        var playerResilience = farmer.resilience + farmer.Read<float>(DataFields.ResonantDefense);
        return 10f / (10f + weaponResilience) * (10f / (10f + playerResilience));
    }
