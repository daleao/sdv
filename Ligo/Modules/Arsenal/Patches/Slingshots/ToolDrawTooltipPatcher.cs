...
internal sealed class ToolDrawTooltipPatcher : HarmonyPatcher
{
        ...
        StringBuilder? overrideText)
    {
        ...
        // write description
        ItemDrawTooltipPatcher.ItemDrawTooltipReverse(__instance, spriteBatch, ref x, ref y, font, alpha, overrideText);
        ...
        {
            var amount =
                $"+{(__instance.GetEnchantmentLevel<RubyEnchantment>() * 0.1f) + __instance.Read<float>(DataFields.ResonantDamage):0%}";
        ...
        {
            var amount =
                $"+{(__instance.GetEnchantmentLevel<AquamarineEnchantment>() * 0.046f) + __instance.Read<float>(DataFields.ResonantCritChance):0%}";
            co = new Color(0, 120, 120);
            Utility.drawWithShadow(
                spriteBatch,
                Game1.mouseCursors,
        ...
        {
            var amount =
                $"+{(__instance.GetEnchantmentLevel<JadeEnchantment>() * (ModEntry.Config.EnableArsenal && ModEntry.Config.Arsenal.RebalancedForges ? 0.5f : 0.1f)) + __instance.Read<float>(DataFields.ResonantCritPower):0%}";
            co = new Color(0, 120, 120);
            Utility.drawWithShadow(
                spriteBatch,
                Game1.mouseCursors,
        ...
        {
            var amount =
                $"+{(__instance.GetEnchantmentLevel<AmethystEnchantment>() * 0.1f) + __instance.Read<float>(DataFields.ResonantKnockback)}:0%";
            co = new Color(0, 120, 120);
            Utility.drawWithShadow(
                spriteBatch,
                Game1.mouseCursors,
        ...
        {
            var amount =
                $"+{(__instance.GetEnchantmentLevel<EmeraldEnchantment>() + __instance.Read<float>(DataFields.ResonantSpeed)) * 0.1f:0%}";
            co = new Color(0, 120, 120);
            Utility.drawWithShadow(
                spriteBatch,
                Game1.mouseCursors,
        ...
        if (__instance.hasEnchantmentOfType<TopazEnchantment>())
        {
            var amount = ModEntry.Config.EnableArsenal && ModEntry.Config.Arsenal.OverhauledDefense
                ? $"+{(__instance.GetEnchantmentLevel<TopazEnchantment>() + __instance.Read<float>(DataFields.ResonantDefense)) * 0.1f:0%}"
