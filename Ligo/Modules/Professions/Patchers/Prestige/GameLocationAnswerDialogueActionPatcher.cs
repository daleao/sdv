...
internal sealed class GameLocationAnswerDialogueActionPatcher : HarmonyPatcher
{
    ...
    {
        var player = Game1.player;
        player.Money = Math.Max(0, player.Money - (int)ModEntry.Config.Professions.PrestigeRespecCost);
    ...
    private static void HandleChangeUlti(Farmer who, string choice)
    {
        ...
        var player = Game1.player;
        player.Money = Math.Max(0, player.Money - (int)ModEntry.Config.Professions.ChangeUltCost);
