...
internal sealed class SetFishQualityCommand : ConsoleCommand
{
    ...
    public override bool CallbackImpl(string trigger, string[] args)
    {
        ...
        var familyCount = Data.ReadAs<int>(nearest, DataKeys.FamilyLivingHere, modId: "DaLion.Professions");
        ...
        if (familyCount > 0)
        {
            ...
            Data.Write(nearest, DataKeys.FamilyQualities, string.Join(',', familyQualities));
        }

        var fishQualities = new int[4];
        fishQualities[newQuality == ObjectQuality.Iridium ? 3 : (int)newQuality] += nearest.FishCount - familyCount;
        Data.Write(nearest, DataKeys.FishQualities, string.Join(',', fishQualities));
