...
internal static class FishPondExtensions
{
    ...
    internal static void AddBonusProduceAmountAndQuality(this FishPond pond)
    {
        ...
        if (produce is not null &&
            !produce.ParentSheetIndex.IsAnyOf(Constants.ROE_INDEX_I, Constants.SQUID_INK_INDEX_I) &&
            !pond.fishType.Value.IsAlgae()) return;
        ...
        if (pond.fishType.Value.IsAlgae())
        {
            ...
            produce = r.NextDouble() < (double) seaweedCount / pond.currentOccupants.Value
                ? new(152, r.Next(seaweedCount))
                : r.NextDouble() < (double) greenAlgaeCount / pond.currentOccupants.Value
                    ? new(153, r.Next(greenAlgaeCount))
                    : r.NextDouble() < (double) seaweedCount / pond.currentOccupants.Value
                        ? new(157, r.Next(whiteAlgaeCount))
                        : null;
        ...
        if (produce is null)
        {
            ...
            if (fish.Name.Contains("Squid"))
                produceId = Constants.SQUID_INK_INDEX_I;
            else if (fish.Name != "Coral")
                produceId = Constants.ROE_INDEX_I;
            else
                produceId = r.Next(Constants.SEAWEED_INDEX_I, Constants.GREEN_ALGAE_INDEX_I + 1);

            if (produceId != Constants.ROE_INDEX_I)
            {
            ...
            else
            {
                ...
                produce = new ColoredObject(Constants.ROE_INDEX_I, 1, c);
