...
- **[Arsenal](Modules/Arsenal)** is the newest and second largest. It overhauls various aspects of both melee weapons and slingshots so as to diversify combat and provide viable alternatives to the ubiquitous sword. Included are all-new enchantments, weapon combos and one new weapon type, as well as novelty features for making legendary weapons feel truly legendary. NOTE: If you enable this module on an existing save, you must run the command `ars initialize` after loading the save to apply the necessary changes to existing weapons. If you then disable this module, you must run the command `overhaul revert_arsenal` to revert existing weapons back to vanilla status. Failure to do so may cause your game to freeze when trying to use certain weapons. Please be aware that this module is in BETA and likely to contain bugs or imbalances.
