namespace DaLion.Redux.Professions.Events.Content;
...
internal sealed class StaticAssetRequestedEvent : AssetRequestedEvent
{
    ...
    internal StaticAssetRequestedEvent(EventManager manager)
        : base(manager)
    {
        ...
        AssetProviders[$"{ModEntry.Manifest.UniqueID}/PrestigeProgression"] = (
            Provide: () => $"assets/sprites/{ModEntry.Config.Professions.PrestigeProgressionStyle}.png",
            Priority: AssetLoadPriority.Medium);
    ...
    private static void EditCursorsLooseSprites(IAssetData asset)
    {
        ...
        editor.PatchImage(Textures.SpriteSheetTx, srcArea, targetArea);
    }
    ...
    private static string ProvideUltimateMeter()
    {
        ...
        if (Redux.Integrations.SveConfig is not null)
        {
            if (Redux.Integrations.SveConfig.Value<bool?>("DisableGaldoranTheme") == false &&
                (Game1.currentLocation?.NameOrUniqueName.IsIn(
                 ...
                 Redux.Integrations.SveConfig.Value<bool?>("UseGaldoranThemeAllTimes") == true))
            {
