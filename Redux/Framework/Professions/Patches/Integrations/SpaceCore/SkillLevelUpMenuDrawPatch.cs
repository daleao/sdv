...
internal sealed class SkillLevelUpMenuDrawPatch : HarmonyPatch
{
    ...
    private static void DrawSubroutine(IClickableMenu menu, int currentLevel, SpriteBatch b)
    {
        ...
        var professionsToChoose = ModEntry.Reflector
            ...
            .Invoke(menu);
        if (!SCProfession.LoadedProfessions.TryGetValue(professionsToChoose[0], out var leftProfession) ||
            !SCProfession.LoadedProfessions.TryGetValue(professionsToChoose[1], out var rightProfession))
        {
            return;
        }
