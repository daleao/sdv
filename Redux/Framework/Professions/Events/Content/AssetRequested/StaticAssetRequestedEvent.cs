...
internal sealed class StaticAssetRequestedEvent : AssetRequestedEvent
{
    ...
    private static string ProvideUltimateMeter()
    {
        ...
        if (Framework.Integrations.SveConfig is not null)
        {
            if (Framework.Integrations.SveConfig.Value<bool?>("DisableGaldoranTheme") == false &&
                (Game1.currentLocation?.NameOrUniqueName.IsIn(
                 ...
                 Framework.Integrations.SveConfig.Value<bool?>("UseGaldoranThemeAllTimes") == true))
            {
