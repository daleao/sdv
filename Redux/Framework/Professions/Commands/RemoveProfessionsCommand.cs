        ...
        foreach (var arg in args)
        {
            if (string.Equals(arg, "all", StringComparison.InvariantCultureIgnoreCase))
            {
                var range = Profession
                    .GetRange()
                    .Concat(SCProfession.LoadedProfessions.Values.Select(p => p.Id))
                    .ToArray();
                ...
                string.Equals(arg, "unknown", StringComparison.InvariantCultureIgnoreCase))
            {
                var range = Game1.player.professions
                    .Where(pid =>
                        ...
                        SCProfession.LoadedProfessions.Values.All(p => pid != p.Id))
                    .ToArray();
            ...
            {
                var customProfession = SCProfession.LoadedProfessions.Values.FirstOrDefault(p =>
