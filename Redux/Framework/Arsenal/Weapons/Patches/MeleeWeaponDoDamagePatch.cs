...
internal sealed class MeleeWeaponDoDamagePatch : HarmonyPatch
{
        ...
        try
        {
            helper
                ...
                .InsertInstructions(
                    ...
                    new CodeInstruction(
                        OpCodes.Call,
                        typeof(MeleeWeaponDoDamagePatch).RequireMethod(nameof(AddResonantDamage))));
        }
        catch (Exception ex)
        {
        ...
        try
        {
            helper
                ...
                .InsertInstructions(
                    ...
                    new CodeInstruction(
                        OpCodes.Call,
                        typeof(MeleeWeaponDoDamagePatch).RequireMethod(nameof(AddResonantKnockback))));
        }
        catch (Exception ex)
        {
        ...
        try
        {
            helper
                ...
                .InsertInstructions(
                    ...
                    new CodeInstruction(
                        OpCodes.Call,
                        typeof(MeleeWeaponDoDamagePatch).RequireMethod(nameof(AddResonantCritChance))));
        }
        catch (Exception ex)
        {
        ...
        try
        {
            helper
                ...
                .InsertInstructions(
                    ...
                    new CodeInstruction(
                        OpCodes.Call,
                        typeof(MeleeWeaponDoDamagePatch).RequireMethod(nameof(AddResonantCritPower))));
        }
        catch (Exception ex)
        {
    ...
    {
        return damage * (1f + weapon.Read<float>(DataFields.ResonantWeaponDamage));
    }

    private static float AddResonantKnockback(float knockback, MeleeWeapon weapon)
    {
        return knockback * (1f + weapon.Read<float>(DataFields.ResonantWeaponKnockback));
    }

    private static float AddResonantCritChance(float critChance, MeleeWeapon weapon)
    {
        return critChance * (1f + weapon.Read<float>(DataFields.ResonantWeaponCritChance));
    }

    private static float AddResonantCritPower(float critPower, MeleeWeapon weapon)
    {
        return critPower * (1f + weapon.Read<float>(DataFields.ResonantWeaponCritPower));
    }
