...
namespace TheLion.AwesomeProfessions
{
	internal class PondQueryMenuDrawPatch : BasePatch
	{
		...
		{
			harmony.Patch(
				AccessTools.Method(typeof(PondQueryMenu), nameof(PondQueryMenu.draw), new[] { typeof(SpriteBatch) }),
		...
		private static bool PondQueryMenuDrawPrefix(ref PondQueryMenu __instance, ref float ____age, ref Rectangle ____confirmationBoxRectangle, ref string ____confirmationText, ref SObject ____fishItem, ref FishPond ____pond, ref bool ___confirmingEmpty, ref string ___hoverText, SpriteBatch b)
		{
			...
			if (!Game1.globalFade)
			{
				...
				bool hasUnresolvedNeeds = ____pond.neededItem.Value != null && ____pond.HasUnresolvedNeeds() && !____pond.hasCompletedRequest.Value;
				string pondNameText = Game1.content.LoadString("Strings\\UI:PondQuery_Name", ____fishItem.DisplayName);
				Vector2 textSize = Game1.smallFont.MeasureString(pondNameText);
				Game1.DrawBox((int)((Game1.uiViewport.Width / 2) - (textSize.X + 64f) * 0.5f), __instance.yPositionOnScreen - 4 + 128, (int)(textSize.X + 64f), 64);
				SUtility.drawTextWithShadow(b, pondNameText, Game1.smallFont, new Vector2((Game1.uiViewport.Width / 2) - textSize.X * 0.5f, (float)(__instance.yPositionOnScreen - 4) + 160f - textSize.Y * 0.5f), Color.Black);
				string displayedText = AwesomeProfessions.Reflection.GetMethod(__instance, name: "getDisplayedText").Invoke<string>();
				int extraHeight = 0;
				if (hasUnresolvedNeeds)
					extraHeight += 116;

				int extraTextHeight = AwesomeProfessions.Reflection.GetMethod(__instance, name: "measureExtraTextHeight").Invoke<int>(displayedText);
				...
				string populationText = Game1.content.LoadString("Strings\\UI:PondQuery_Population", string.Concat(____pond.FishCount), ____pond.maxOccupants.Value);
				...
				int slotsToDraw = ____pond.maxOccupants.Value;
				float slotSpacing = 11f;
				int x = 0;
				int y = 0;
				...
				if (hasUnresolvedNeeds)
				{
					...
					string bringText = Game1.content.LoadString("Strings\\UI:PondQuery_StatusRequest_Bring");
					textSize = Game1.smallFont.MeasureString(bringText);
					int leftX = __instance.xPositionOnScreen + 88;
					float textX = leftX;
					float iconX = textX + textSize.X + 4f;
