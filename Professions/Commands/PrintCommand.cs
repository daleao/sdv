    ...
    : ConsoleCommand(handler)
{
    ...
    private void PrintFishPokedex()
    {
        ...
        var priceMultiplier = Game1.player.HasProfession(Profession.Angler)
            ? CurrentCulture(
                $"{Math.Min((numMaxSizedCaught * Config.AnglerPercentagePriceScalingPerFish / 100f) + (numLegendaryCaught * Config.AnglerPercentagePriceScalingPerFish / 100f * 4f), Config.AnglerPriceBonusCeiling):0%}")
