...
{
    private static readonly Lazy<Func<Multiplayer>> GetMultiplayer = new(() =>
        typeof(Game1)
            .RequireField("multiplayer")
        ...
        GameLocation __instance, Vector2 tileLocation, int radius, Farmer? who)
    {
        ...
        foreach (var tile in circle.Tiles)
        {
            ...
            if (Lookups.ResourceFromStoneId.TryGetValue(
        ...
        if (!who.IsLocalPlayer || !isDemolitionist || !ModEntry.Config.EnableGetExcited)
        {
            return;
        }
        ...
        {
            ModEntry.State.DemolitionistExcitedness = 4;
        }

        if (distanceFromEpicenter < radius + 1)
        {
            ModEntry.State.DemolitionistExcitedness += 2;
        }
