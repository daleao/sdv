...
public sealed class Chord : IChord
{
    ...
    {
        this.ResonanceByGemstone.ForEach(pair => pair.Key.Resonate((float)pair.Value, who));
    ...
    {
        this.ResonanceByGemstone.ForEach(pair => pair.Key.Dissonate((float)pair.Value, who));
        who.MagneticRadius += this._magnetism;
        if (this.LightSource is null)
        {
            return;
