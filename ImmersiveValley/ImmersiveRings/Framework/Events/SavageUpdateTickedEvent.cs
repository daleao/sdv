...
{
    private const int SavangeRingIndex = 523;
        ...
        : base(manager)
    {
        ...
        this._buffSource =
            ModEntry.ModHelper.GameContent.Load<Dictionary<int, string>>("Data/ObjectInformation")[SavangeRingIndex]
                .Split('/')[0];
    ...
    {
        var savageness = ModEntry.State.SavageExcitedness;
        if (savageness <= 0)
        {
            this.Disable();
        }
        ...
        ModEntry.State.SavageExcitedness = Math.Max(0, savageness - buffDecay);
    }
}
