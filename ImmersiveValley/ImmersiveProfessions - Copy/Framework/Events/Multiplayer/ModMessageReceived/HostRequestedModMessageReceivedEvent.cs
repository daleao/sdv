    ...
    protected override void OnModMessageReceivedImpl(object? sender, ModMessageReceivedEventArgs e)
    {
        ...
        switch (request)
        {
            case "Conservationism":
                ...
                Manager.Hook<ConservationismDayEndingEvent>();
                break;
            case "HuntIsOn":
                Log.D($"Prestiged treasure hunter {who.Name} is hunting for treasure.");
                ModEntry.Host.PlayersHuntingTreasure.Add(who.UniqueMultiplayerID);
                Manager.Hook<PrestigeTreasureHuntUpdateTickedEvent>();
                break;
            case "HuntIsOff":
                Log.D($"{who.Name}'s hunt has ended.");
                ModEntry.Host.PlayersHuntingTreasure.Remove(who.UniqueMultiplayerID);
                break;
        }
    }
}