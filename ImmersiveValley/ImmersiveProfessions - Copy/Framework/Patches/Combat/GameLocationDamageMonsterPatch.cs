...
internal sealed class GameLocationDamageMonsterPatch : DaLion.Common.Harmony.HarmonyPatch
{
        ...
        Monster monster, Farmer who)
    {
        ...
        {
            ModEntry.Player.SecondsOutOfCombat = 0;

            if (who.CurrentTool is MeleeWeapon weapon &&
                ModEntry.Player.RegisteredUltimate is Frenzy frenzy && monster.Health <= 0)
            {
                if (frenzy.IsActive)
                {
                    // lifesteal
                    ...
                    ++ModEntry.Player.BruteKillCounter;
                }
                else
                {
                    // increment ultimate
        ...
        if (who.HasProfession(Profession.Poacher))
        {
            if (who.CurrentTool is MeleeWeapon && didCrit)
            {
                ...
                if (ModEntry.Player.RegisteredUltimate is Ambush { IsActive: false } ambush1)
                    ambush1.ChargeValue += critMultiplier;
            }

            if (ModEntry.Player.RegisteredUltimate is Ambush ambush2)
            {
        ...
        {
            var applied = ModEntry.Player.AppliedPiperBuffs;
        ...
        if (ModEntry.Player.RegisteredUltimate is Concerto { IsActive: false } concerto)
        {
