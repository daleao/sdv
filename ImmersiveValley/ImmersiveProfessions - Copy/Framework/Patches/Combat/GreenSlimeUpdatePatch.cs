...
internal sealed class GreenSlimeUpdatePatch : DaLion.Common.Harmony.HarmonyPatch
{
    ...
    {
        if (!ModEntry.Player.PipedSlimes.Contains(__instance)) return;
        ...
        foreach (var monster in __instance.currentLocation.characters.OfType<Monster>().Where(m => !m.IsSlime()))
        {
            ...
            {
                fakeFarmer = ModEntry.Host.FakeFarmers[fakeFarmerId] =
            ...
            ModDataIO.Write(monster, "Aggroed", true.ToString());
            ModDataIO.Write(monster, "Aggroer", __instance.GetHashCode().ToString());
