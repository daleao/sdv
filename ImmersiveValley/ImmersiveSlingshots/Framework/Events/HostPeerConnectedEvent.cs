...
internal sealed class HostPeerConnectedEvent : PeerConnectedEvent
{
    ...
    {
        if (!e.Peer.IsSplitScreen || !e.Peer.ScreenID.HasValue || !ModEntry.Config.FaceMouseCursor) return;

        ModEntry.Events.EnableForScreen<SlingshotButtonPressedEvent>(e.Peer.ScreenID.Value);
    }
}