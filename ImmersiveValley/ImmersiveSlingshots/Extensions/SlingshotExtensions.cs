...
{
    private static readonly Lazy<Action<Tool, Farmer>> SetLastUser = new(() =>
        typeof(Tool)
            .RequireField("lastUser")
            .CompileUnboundFieldSetterDelegate<Tool, Farmer>());
    ...
    {
        if (Game1.fadeToBlack || ModEntry.State.SlingshotCooldown > 0)
        {
            return;
        }
    ...
    public static void DoDamage(this Slingshot slingshot, int x, int y, Farmer who)
    {
        ...
        SetLastUser.Value(dummyWeapon, who);
