...
internal sealed class CraftingRecipeDrawRecipeDescriptionPatcher : HarmonyPatcher
{
    ...
    internal CraftingRecipeDrawRecipeDescriptionPatcher(Harmonizer harmonizer)
        : base(harmonizer)
    {
        ...
        CraftingRecipe __instance, SpriteBatch b, Vector2 position, int width, IList<Item> additional_crafting_items)
    {
        ...
        try
        {
            ...
            {
                var foundInBackpack = id.IsRingId()
                ...
                {
                    foundInContainers = id.IsRingId()
                        ? Game1.player.GetRingItemCount(id, additional_crafting_items)
                        : Game1.player.getItemCountInList(additional_crafting_items, id);
                    if (remaining > 0)
                    {
                ...
                var ingredientNameText = __instance.getNameFromIndex(id);
                var drawColor = remaining <= 0 ? Game1.textColor : Color.Red;
                var dataOrErrorItem = ItemRegistry.GetDataOrErrorItem(__instance.getSpriteIndexFromRawIndex(id));
