    ...
    : RenderedWorldEvent(manager ?? CoreMod.EventManager)
{
    ...
    protected override void OnRenderedWorldImpl(object? sender, RenderedWorldEventArgs e)
    {
        ...
        var mapWidth = Game1.currentLocation.Map.Layers[0].LayerWidth;
        var mapHeight = Game1.currentLocation.Map.Layers[0].LayerHeight;
        Func<Vector2, bool> isMineShaftTileSpawnable = tile =>
            location.isTileOnMap(tile) && (location as MineShaft)!.isTileOnClearAndSolidGround(tile) && !location.IsTileBlockedBy(tile, CollisionMask.Buildings | CollisionMask.Furniture |
        ...
        for (var x = 0; x < mapWidth; x++)
        {
            ...
            {
                bb = new Rectangle(x * 64 + 8, y * 64 + 8, 48, 48);
                if (!Game1.viewport.Intersects(new xTile.Dimensions.Rectangle(bb.X, bb.Y, bb.Width, bb.Height)))
                {
                    continue;
                }
                ...
                bb.X -= Game1.viewport.X;
                bb.Y -= Game1.viewport.Y;
                if (isMineShaftTileSpawnable(new Vector2(x, y)))
                {
                    bb.Highlight(Color.Green * 0.5f, e.SpriteBatch);
                }
                else
